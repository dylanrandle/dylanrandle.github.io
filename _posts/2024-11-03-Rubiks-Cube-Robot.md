...
- **Plan:** Imperceptible in the video is the execution of the planning phase, which leverages [Kociemba's algorithm](https://kociemba.org) to plan a sequence of moves that solves the Rubik's cube, given an initial state. This algorithm is incredibly fast, taking on the order of milliseconds to compute a near-optimal solution.
